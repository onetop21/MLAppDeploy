...
if [[ "$1" == "build" ]]; then
    ...
    docker service create -q --name mlad-service -e MLAD_DEBUG=$DEBUG -e PYTHONUNBUFFERED=1 -p 8440:8440 --mount 'type=bind,src=/var/run/docker.sock,dst=/var/run/docker.sock' mlad/service \
